...
if (firstName && lastName && emailSignUp && passwordSignUp && btnSignUp) {
...
const inputTabUser = document.querySelector('.input-tab-user');
const btnSearchTabUser = document.querySelector('.btn-search-tab-user');
const userListContainer = document.querySelector('.users');
const tableTabUser = document.querySelector('.table-tab-user');
const alertErrorUser = document.querySelector('.alert-error-user');
if (inputTabUser && btnSearchTabUser && userListContainer && alertErrorUser) {
    btnSearchTabUser.addEventListener('click', () => {
        userListContainer.innerHTML = '';
        if (inputTabUser.value.length === 0) {
            users.map((value) => {
                tableTabUser.style.display = 'block';
                alertErrorUser.innerHTML = '';
                userListContainer.innerHTML += `
    <tr>
      ...
      <td>${value.email}</td>
            ...
            users
                .filter(
                    (user) =>
                        user.first_name.toLocaleLowerCase().includes(inputTabUser.value.toLocaleLowerCase()) ||
                        user.last_name.toLocaleLowerCase().includes(inputTabUser.value.toLocaleLowerCase()) ||
                        user.email.toLocaleLowerCase().includes(inputTabUser.value.toLocaleLowerCase())
                )
                .map((user) => result.push(user));
            if (result.length === 0) {
                tableTabUser.style.display = 'none';
                alertErrorUser.innerHTML = `<div class="alert alert-danger flex items-center gap-2" role="alert">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/>
</svg>
  <p>Không thể tìm được user với kết quả đã tìm</p>
</div>`;
                return;
            }
            result.map((value) => {
...
const tabPostContainer = document.querySelector('.tab-post-container');
if (tabPostContainer) {
    const postWithName = posts.map((post) => {
    ...
    console.log({ postWithName });
    let result = '';
    postWithName.map((post) => {
        result += `<tr>
    ...
    tabPostContainer.innerHTML = result;
}

const inputTabPost = document.querySelector('.input-tab-post');
const btnSearchTabPost = document.querySelector('.btn-search-tab-post');
const tableTabPost = document.querySelector('.table-tab-post');
const postListContainer = document.querySelector('.post-list-container');
const alertErrorPost = document.querySelector('.alert-error-post');
if (inputTabPost && btnSearchTabPost && postListContainer) {
    btnSearchTabPost.addEventListener('click', () => {
        let result = [];
        posts.filter((post) => post.id === Number(inputTabPost.value)).map((post) => result.push(post));
        const resultWithName = result.map((value) => {
            const user = users.find((user) => user.id === value.user_id);
            return {
                ...value,
                fullName: user ? `${user.first_name} ${user.last_name}` : 'unknown',
            };
        });
        console.log({ resultWithName });
        if (resultWithName.length === 0) {
            postListContainer.innerHTML = '';
            tableTabPost.style.display = 'none';
            alertErrorPost.innerHTML = `<div class="alert alert-danger flex items-center gap-2" role="alert">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/>
</svg>
  <p>Không thể tìm được post với kết quả đã tìm</p>
</div>`;
        } else {
            resultWithName.map((value) => {
                postListContainer.innerHTML = '';
                alertErrorPost.innerHTML = '';
                tableTabPost.style.display = 'block';
                postListContainer.innerHTML += `
                  <tr>
      <th scope="row" class="text-start">${value.id}</th>
      <td>${value.title}</td>
      <td class="w-[500px]">${value.content}</td>
     <td class="align-top ">
  <img src="${value.image}" />
</td>
...
const inputTabPostEmail = document.querySelector('.input-tab-post-email');
const btnSearchTabPostEmail = document.querySelector('.btn-search-tab-post-email');
const tableTabPostEmail = document.querySelector('.table-tab-post-email');
const postListEmailContainer = document.querySelector('.post-list-email-container');
const alertErrorPostEmail = document.querySelector('.alert-error-post-email');
if (inputTabPostEmail && btnSearchTabPostEmail && tableTabPostEmail && postListEmailContainer && alertErrorPostEmail) {
    btnSearchTabPostEmail.addEventListener('click', () => {
        const postWithName = posts.map((post) => {
            const user = users.find((user) => user.id === post.user_id);
            return {
                ...post,
                fullName: user ? `${user.first_name} ${user.last_name}` : 'unknown',
                email: user ? `${user.email}` : 'unkown',
            };
        });
        console.log({ postWithName });
        const isExitEmail = users.some((user) => user.email === inputTabPostEmail.value);
        if (!isExitEmail) {
            tableTabPostEmail.style.display = 'none';
            postListEmailContainer.innerHTML = '';
            alertErrorPostEmail.innerHTML = `<div class="alert alert-danger flex items-center gap-2" role="alert">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
  <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0M7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0z"/>
</svg>
  <p>Không thể tìm được post với kết quả đã tìm</p>
            ...
            alertErrorPostEmail.innerHTML = '';
            tableTabPostEmail.style.display = 'block';
            postWithName
                .filter((value) => value.email === inputTabPostEmail.value)
                .map((value) => {
                    postListEmailContainer.innerHTML += `
                        <tr>
      <th scope="row" class="text-start">${value.id}</th>
      <td>${value.title}</td>
      <td class="w-[450px]">${value.content}</td>
     <td class="align-top ">
  <img src="${value.image}" />
</td>
      <td>${value.email}</td>
